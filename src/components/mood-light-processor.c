...
static int
ambitv_mood_light_processor_handle_frame(
   ...
   struct ambitv_mood_light_processor* mood =
      (struct ambitv_mood_light_processor*)component->priv;
...
static int
ambitv_mood_light_processor_update_sink(
   ...
   struct ambitv_sink_component* sink)
{
   ...
   struct ambitv_mood_light_processor* mood =
      (struct ambitv_mood_light_processor*)processor->priv;
...
static int
ambitv_mood_light_processor_configure(struct ambitv_processor_component* mood, int argc, char** argv)
{
   int c, ret = 0;

   struct ambitv_mood_light_processor* mood_priv =
      (struct ambitv_mood_light_processor*)mood->priv;
   if (NULL == mood_priv)
      return -1;
...
static void
ambitv_mood_light_processor_print_configuration(struct ambitv_processor_component* component)
{
   struct ambitv_mood_light_processor* mood =
      (struct ambitv_mood_light_processor*)component->priv;
...
static void
ambitv_mood_light_processor_free(struct ambitv_processor_component* component)
{
   free(component->priv);
}
...
{
   struct ambitv_processor_component* mood_light_processor =
      ambitv_processor_component_create(name);

   if (NULL != mood_light_processor) {
      struct ambitv_mood_light_processor* priv =
         (struct ambitv_mood_light_processor*)malloc(sizeof(struct ambitv_mood_light_processor));
...
errReturn:
   ambitv_processor_component_free(mood_light_processor);
   return NULL;
}
