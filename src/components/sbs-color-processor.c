...
#define LOGNAME   "sbs-color-processor: "

struct ambitv_sbs_processor_priv {
   void* frame;
...
static int
ambitv_sbs_color_processor_handle_frame(
   ...
   struct ambitv_sbs_processor_priv* edge =
      (struct ambitv_sbs_processor_priv*)component->priv;
...
static int
ambitv_sbs_color_processor_update_sink(
   ...
   struct ambitv_sink_component* sink)
{
   ...
   struct ambitv_sbs_processor_priv* edge =
      (struct ambitv_sbs_processor_priv*)processor->priv;
   ...
   if (sink->f_num_outputs && sink->f_set_output_to_rgb && sink->f_map_output_to_point) {
      ...
      for (i=0; i<n_out; i++) {
         ...
         if (0 == sink->f_map_output_to_point(sink, i, edge->width, edge->height, &x, &y)) {
            ...
            sink->f_set_output_to_rgb(sink, i, rgb[0], rgb[2], rgb[1]);
         }
      }
   } else
      ret = -1;
...
static int
ambitv_sbs_color_processor_configure(struct ambitv_sbs_processor_priv* edge, int argc, char** argv)
{
   int c, ret = 0;
...
static void
ambitv_sbs_color_processor_print_configuration(struct ambitv_processor_component* component)
{
   struct ambitv_sbs_processor_priv* edge =
      (struct ambitv_sbs_processor_priv*)component->priv;
...
static void
ambitv_sbs_color_processor_free(struct ambitv_processor_component* component)
{
   free(component->priv);
}

struct ambitv_processor_component*
ambitv_sbs_color_processor_create(const char* name, int argc, char** argv)
{
   struct ambitv_processor_component* edge_processor =
      ambitv_processor_component_create(name);

   if (NULL != edge_processor) {
      struct ambitv_sbs_processor_priv* priv =
         (struct ambitv_sbs_processor_priv*)malloc(sizeof(struct ambitv_sbs_processor_priv));
      memset(priv, 9, sizeof(struct ambitv_sbs_processor_priv));
      ...
      edge_processor->f_print_configuration  = ambitv_sbs_color_processor_print_configuration;
      edge_processor->f_consume_frame        = ambitv_sbs_color_processor_handle_frame;
      edge_processor->f_update_sink          = ambitv_sbs_color_processor_update_sink;
      edge_processor->f_free_priv            = ambitv_sbs_color_processor_free;
      
      if (ambitv_sbs_color_processor_configure(priv, argc, argv) < 0) {
         goto errReturn;
      }
   }
