...
#define LOGNAME   "ttb-color-processor: "

struct ambitv_ttb_processor_priv {
   void* frame;
...
static int
ambitv_ttb_color_processor_handle_frame(
   ...
   struct ambitv_ttb_processor_priv* edge =
      (struct ambitv_ttb_processor_priv*)component->priv;
...
static int
ambitv_ttb_color_processor_update_sink(
   ...
   struct ambitv_sink_component* sink)
{
   ...
   struct ambitv_ttb_processor_priv* edge =
      (struct ambitv_ttb_processor_priv*)processor->priv;
...
static int
ambitv_ttb_color_processor_configure(struct ambitv_ttb_processor_priv* edge, int argc, char** argv)
{
   int c, ret = 0;
...
static void
ambitv_ttb_color_processor_print_configuration(struct ambitv_processor_component* component)
{
   struct ambitv_ttb_processor_priv* edge =
      (struct ambitv_ttb_processor_priv*)component->priv;
...
static void
ambitv_ttb_color_processor_free(struct ambitv_processor_component* component)
{
   free(component->priv);
}

struct ambitv_processor_component*
ambitv_ttb_color_processor_create(const char* name, int argc, char** argv)
{
   struct ambitv_processor_component* edge_processor =
      ambitv_processor_component_create(name);

   if (NULL != edge_processor) {
      struct ambitv_ttb_processor_priv* priv =
         (struct ambitv_ttb_processor_priv*)malloc(sizeof(struct ambitv_ttb_processor_priv));
      memset(priv, 9, sizeof(struct ambitv_ttb_processor_priv));
      ...
      edge_processor->f_print_configuration  = ambitv_ttb_color_processor_print_configuration;
      edge_processor->f_consume_frame        = ambitv_ttb_color_processor_handle_frame;
      edge_processor->f_update_sink          = ambitv_ttb_color_processor_update_sink;
      edge_processor->f_free_priv            = ambitv_ttb_color_processor_free;
      
      if (ambitv_ttb_color_processor_configure(priv, argc, argv) < 0) {
         goto errReturn;
      }
   }
