...
static int
ambitv_timer_source_start(struct ambitv_source_component* timer)
{
   return 0;
}

static int
ambitv_timer_source_stop(struct ambitv_source_component* timer)
{
   return 0;
}

static int
ambitv_timer_source_loop_iteration(struct ambitv_source_component* timer)
{
   struct ambitv_timer_priv* timer_priv =
      (struct ambitv_timer_priv*)timer->priv;
...
static int
ambitv_timer_source_configure(struct ambitv_source_component* timer, int argc, char** argv)
{
   int c, ret = 0;
...
static void
ambitv_timer_source_print_configuration(struct ambitv_source_component* component)
{
   struct ambitv_timer_priv* timer_priv =
      (struct ambitv_timer_priv*)component->priv;
...
static void
ambitv_timer_source_free(struct ambitv_source_component* component)
{
}
...
ambitv_timer_source_create(const char* name, int argc, char** argv)
{
   ...
   if (NULL != timer) {
      struct ambitv_timer_priv* timer_priv =
         (struct ambitv_timer_priv*)malloc(sizeof(struct ambitv_timer_priv));
      if (NULL == timer_priv)
         goto errReturn;
...
errReturn:
   ambitv_source_component_free(timer);
